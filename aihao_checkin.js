...
let cookies = $.isNode() ? (process.env.aihao_cookie ? process.env.aihao_cookie : "") : ($.getdata('aihao_cookie') ? $.getdata('aihao_cookie') : "")
let cookieArr = []
let result = ""
!(async() => {      
        if (!cookies) {
            $.msg($.name,  '【提示】请先获取爱好论坛ck')
            return;
        }
        timeZone = new Date().getTimezoneOffset() / 60;
        timestamp = Date.now() + (8 + timeZone) * 60 * 60 * 1000;
        //timestamp = Date.now() + (timeZone) * 60 * 60 * 1000;
...
function aihao() {
    return new Promise(async (resolve) => {
      try {
        ...
        // let cookies = $.getdata('aihao_cookie');
        let header = { headers: { "cookie": cookies } };
        for (i of [1, 2, 3, 4]) {
